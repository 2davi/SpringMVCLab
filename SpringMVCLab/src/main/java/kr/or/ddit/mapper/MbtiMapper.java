...
/**
 * Persistence Layer: 테이블을 대상으로 하는 CRUD 메소드(사용할 쿼리 반영)가 기본 요건
 * ***MyBatis에서는 Mapper 인터페이스와 Mapper.xml이 1:1 구조로 개발됨.
 * 	  프레임워크 내부에서 리플렉션을 하기 위해서는 해당 인터페이스를 찾아내는 조건이 필요하고,
 * 	  그 조건을 xml의 namespace 속성으로 설정함.
 * ***인터페이스의 메소드와 xml 내부의 쿼리 하나가 1:1 구조로 개발됨.
 * 	  쿼리의 ID를 기준으로 메소드의 1:1 구조가 형성됨.
 * 	  
 */
@Mapper
public interface MbtiMapper {
	...
	/**
	 * 다건 조회, 데이터 매퍼는 다건조회에서 null값을 반환하지 않음. (empty list)
	 * @return
	 */
