 ...
 * 	  ex) ProdVOBuilder
 * 4. Builder 내부에서 build() 메서드로 객체를 생성함.
 * 5. build 대상이 되는 객체가 가진 프로퍼티를 결정할 수 있는 setter 역할 메서드 구현.
 ...
 * 	  똑같은 테이블일지라도 중심을 어디에 두느냐에 따라 관계는 달라진다.
 * 2. 조인 쿼리 작성 ->.sql workSheet
 * 3. 각 테이블을 대상으로 한 VO 정의
 * 	  ProdVO, LprodVO, BuyerVO
 * 4. 테이블의 구조를 VO에 반영,
 * 	  
 * 	  ex) PROD(1) : BUYER(1)
 * 	 	MAIN이 되는 테이블부터 VO -> ProdVO
 * 	 	has 관계에 놓일 BuyerVO -> ProdVO has A BuyerVO
 * 	  ex)BUYER(1) : PROD(N)
 * 	 	BuyerVO has Many ProdVO
 * 5. resultType 대신 resultMap을 통해 수동 바인드 설정.
 * 	  1:1 ->(hasA)	 -> association으로 바인드
 * 	  1:N ->(hasMany)-> collection으로 바인드
 * 	  
 */
